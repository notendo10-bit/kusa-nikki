  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, background: '#5a9e22', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <span style={{ color: '#c8f0a0', fontSize: 12 }}>下曽我</span>
      </div>
      {selected && (
        <div style={{ position: 'absolute', bottom: 100, left: '50%', transform: 'translateX(-50%)', background: '#fff', borderRadius: 20, padding: '16px 20px', border: '2px solid #c0dd97', minWidth: 200, textAlign: 'center' }}>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1.5px solid #e8d5b0', display: 'flex' }}>
        {[{ icon: '🗺', label: 'マップ' }, { icon: '📷', label: '撮影' }, { icon: '📋', label: '記録' }].map((item, i) => (
          <button key={i} style={{ flex: 1, padding: '12px 4px 16px', border: 'none', background: 'transparent', fontSize: 10, fontWeight: 'bold', color: i === 0 ? '#5a9e22' : '#7a5c2e', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            <span style={{ fontSize: 22 }}>{item.icon}</span>{item.label}
          </button>
        ))}
      </div>
      <button style={{ position: 'absolute', bottom: 80, right: 20, width: 56, height: 56, borderRadius: '50%', background: '#5a9e22', border: '3px solid #fff', color: '#fff', fontSize: 28, cursor: 'pointer' }}>+</button>
    </div>
  );
}
